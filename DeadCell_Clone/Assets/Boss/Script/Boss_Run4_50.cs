...
public class Boss_Run4_50 : StateMachineBehaviour
{
    Transform player;
    Rigidbody2D rb;
    ...
    override public void OnStateUpdate(Animator animator, AnimatorStateInfo stateInfo, int layerIndex)
    {
        ...
        if (Vector2.Distance(player.position, rb.position) <= attackRange && (Vector2.Distance(player.position, rb.position)>= attackcombo))
        {
        ...
        if (Vector2.Distance(player.position, rb.position) <= attackcombo)
        {
            c.randomNum = Random.value;
